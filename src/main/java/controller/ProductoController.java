...
public class ProductoController {
    ...
    public ResponseEntity<List<ProductoDTO>> all() {
        List<ProductoDTO> list = service.findAll().stream().map(EntityDtoMapper::toDto).collect(Collectors.toList());
        return ResponseEntity.ok(list);
    }

    @GetMapping("/{id}")
    public ResponseEntity<ProductoDTO> get(@PathVariable Long id) {
        return ResponseEntity.ok(EntityDtoMapper.toDto(service.findById(id)));
    }

    @PostMapping
    @PreAuthorize("hasRole('admin')")
    public ResponseEntity<ProductoDTO> create(@Valid @RequestBody ProductoDTO dto) {
        Producto saved = service.create(EntityDtoMapper.toEntity(dto));
    ...
    public ResponseEntity<ProductoDTO> update(@PathVariable Long id, @Valid @RequestBody ProductoDTO dto) {
        Producto updated = service.update(id, EntityDtoMapper.toEntity(dto));
